...
4. Resolve and validate OS paths in the CLI, including overlap and output
   policy checks.
5. Read filesystem inputs into catalogs, construct a typed byte/catalog domain
   request, await the domain API, and persist returned catalogs in the CLI.
...
```text
conkit check all --source DIR --contracts DIR --output FILE [MODE]
conkit check signatures --source DIR --contracts DIR --output FILE [MODE]
...
preserve the current recovery, immutable-baseline, complete-domain-result, and
writer reconciliation flow:
